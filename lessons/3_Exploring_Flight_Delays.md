...
Let it run. Then look at your neighbor's screen.

🔔 **Question:** Same data, same prompt. Did you get the same plot? What does that tell you about working with agents?
...
Most flights are early or on time. But look at that right tail - a small number of flights are catastrophically late.
...
We see here one of the most reliable findings in air travel: delays build up over the day. Early planes leave on time. By evening, the day's problems have piled up.
...
Check the screen: your row count and delay rate should **exactly match** the instructor's.
